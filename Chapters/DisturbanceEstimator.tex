...
Increasing to $Q = 100000$ yields \ref{fig:Kalman_residual_Q01R100000}, the slow dynamics of the consumer patter is still tracked, and the residual plot shows a noticeable spike when the leak is introduced. The leak gradually develops over one hour, with a maximum magnitude equivalent to a flow increase corresponding to $33\%$ of max flow. Quite a large leak is chosen such that the residual tendency can be seen visually, a moving average algorithm is shown to easily find smaller leaks in the next section. 
