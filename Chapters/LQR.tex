...
and therefore suggests that \cref{eq:LagrangeProblemDeviation} penalizes deviations from exactly the equilibrium point $\{x_e,u_{op}\}$. This is a quite attractive quality when contextualised by the Hartman-Grobman theorem, as we expect our linearised model dynamics to closely\footnote{Exactly, in fact, in some indeterminately sized region} approximate the real system dynamics in some region around precisely this equilibrium point. 
